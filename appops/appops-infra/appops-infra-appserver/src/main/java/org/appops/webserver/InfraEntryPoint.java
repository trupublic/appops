...
class InfraEntryPoint.Use this
class when
you			want
to			use
a			standalone
...
public class InfraEntryPoint extends HttpServlet {
			...
			private static Logger		logger				= Logger
					.getLogger(InfraEntryPoint.class.getCanonicalName());
			...
			public String getOperationResult(String query) {
				String result = null;
				try (Socket currentClientSocket = new Socket(InetAddress.getByName(SERVER_IP),
						InfraEntryPoint.SERVER_PORT)) {

					if (currentClientSocket.isConnected()) {
						PrintWriter writer = new PrintWriter(currentClientSocket.getOutputStream(), true);
						BufferedReader reader = new BufferedReader(
								new InputStreamReader(currentClientSocket.getInputStream()));
			...
			/**
			 * @see HttpServlet#doPost(HttpServletRequest request, HttpServletResponse Needs to be tested response)
			 */
