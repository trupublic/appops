...
/**
 * use the two utility methods to find either a set of classes or a setof interfaces annotated with a specific type
 * whose class you specify in the parameter Classes are found using two different system environment variables i.e.
 * java.classpath and java.class.path
 * 
...
public class AnnotatedClassScanner {
	 ...
	 * 
	 * @param t
	 *            class object representing the annotation
	 * @return
	 * @throws IOException
	 */
	...
	public Set<Class<?>> getClassesAnnotatedWith(Class<?> t) throws IOException {
		...
		AnnotatedTypeDiscoveryListener listener = new AnnotatedTypeDiscoveryListener(
				new String[] { t.getCanonicalName() });
	...
	public Set<Class<?>> getInterfacesAnnotatedWith(Class<?> t) throws IOException {
		...
		AnnotatedTypeDiscoveryListener listener = new AnnotatedInterfaceDiscoveryListener(
				new String[] { t.getCanonicalName() });
		discoverer.addAnnotationListener(listener);
		discoverer.discover();
		return listener.getDiscovered();
	...
	static class AnnotatedTypeDiscoveryListener implements ClassAnnotationDiscoveryListener {
		...
		/**
		 * @param annotations
		 *            canonical names of annotations to look for
		 */
